...
module PhaserSpine {
        ...
        export class SpinePlugin extends Phaser.Plugin {
            ...
            private addSpineLoader() {
                (<PhaserSpine.SpineLoader>Phaser.Loader.prototype).spine = function(key: string, url: string, scalingVariants?: string[], fileType?: string) {
                            ...
                            if (cacheKey === atlasKey) {
                                let atlas = new spine.Atlas(this.game.cache.getText(cacheKey), {
                                    load: (page: any, file: string, atlas: spine.Atlas) => {
                                        ...
                                        (<PhaserSpine.SpineLoader>this).image(file, cacheData.basePath + '/' + file.substr(0, file.lastIndexOf('.')) + variant + '.' + fileType);
                                    }
                                });
                            }
                        });
